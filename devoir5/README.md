...
Иногда (ты может встречал), ключи имеббт короткую и блинную форму одновременно,
например `-h, --help`, `-f, --format`.

Часто скрипт имеет и ключи (с парамтрами для ключа илил без) и параметры одновременно,
обычно используют специальную комбинацию символов `--`.
Может быть другая, это зависит как обрабатываются аргументы командной строки,
...
На примере пр запуске `script.sh -f long -- Bonsoir`
...
Пробуем: `./banniere.sh -mt '+' -mg 20 -ep 10  -- Bonsoir`
